        ...
        llm: (() => {
          ...
          if (hasMcps) {
            const mcpNames = mcps
              ...
              .join(", ");
            const hasGitHub = mcps.some((mcp: any) =>
              (mcp.name || "").toLowerCase().includes("github")
            );
            ...
            if (hasGitHub) {
              systemPrompt += `\n\nGITHUB REPOSITORY MANAGEMENT:\nWhen using GitHub tools to publish code:\n- If the user asks to "publish or deploy to GitHub" or similar, create ONE repository and push the code to it\n- For ALL subsequent changes or updates in the same session, ALWAYS update the EXISTING repository instead of creating a new one\n- NEVER create multiple repositories for the same project/session\n- If you need to update code, use the existing repository that was created earlier in the conversation\n- Only create a NEW repository if the user explicitly asks for a separate/new repository or a different project\n- Remember: One project = One repository per session, update the existing one for changes\n\nGITHUB PAGES DEPLOYMENT:\nWhen the user asks to "publish using GitHub Pages", "enable GitHub Pages", "deploy to GitHub Pages", or similar:\n- You MUST actually deploy to GitHub Pages by creating a gh-pages branch in the repository\n- Create the gh-pages branch and push the code to it - this automatically enables GitHub Pages deployment\n- Do NOT just acknowledge the request - you must execute it immediately\n- Use the repository that was created earlier in the session (do not create a new one)\n- After pushing to gh-pages branch, provide the GitHub Pages URL to the user (format: https://[username].github.io/[repo-name]/)\n- Be persistent and complete the action in one go, don't wait for confirmation or ask again`;
            }
          }
